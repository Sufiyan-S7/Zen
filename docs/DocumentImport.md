...
**Superseded August 9, 2026 (Day 18):** `PDF_EXTRACTION_UNAVAILABLE` no longer applies. PDF is now extracted through `readPdfDocument` per [docs/PDFExtraction.md](PDFExtraction.md); it fails closed with `PDF_PASSWORD_PROTECTED`, `PDF_MALFORMED`, `PDF_NO_TEXT_LAYER`, or `PDF_EXTRACTION_FAILED` instead. All other constraints on this page (native-picker-only selection, size/batch limits, local-only storage, no source-file copying, redacted activity logging) are unchanged and apply to PDF the same as any other supported type.
