...
| An unregistered action id is rejected before execution | Call `proposeTask` with an action id not in the registry; expect it to throw synchronously during step validation, before a task (and therefore a `taskId`) is ever created. Zero audit records result -- every audit record is scoped to a `taskId` per Section 4, and a proposal rejected at this stage never reaches one. (Corrected Aug 2026: the original wording called for a `outcome: "failed"` audit record for the rejected attempt itself, which is structurally impossible since `proposeTask` throws before a `taskId` exists -- see `task-executor.js`'s `proposeTask`. This row was reworded to match that reality rather than changing `proposeTask` to synthesize a taskId-less audit record.) |
