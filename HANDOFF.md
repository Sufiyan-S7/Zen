...
> decisions.** Skim its last 2‚Äì3 entries alongside this file at the start of any new session.
...
## Current Status ‚Äî August 6, 2026
...
#### Day 10 ‚Äî local memory complete (August 7, 2026)
...
#### Day 12 ‚Äî selected text-document import complete (August 7, 2026)
...
- August 7, 2026 Day 12 display bug fixed: imports succeeded, but `renderDocuments` shadowed the browser `document` object with a document-record parameter, so the list renderer threw while creating each row and showed the misleading ‚Äúcould not load‚Äù message. The renderer now uses distinct item-record names; imported TXT files will display after restarting Zen.
...
### Latest checkpoint ‚Äî August 7, 2026

#### Day 7 ‚Äî release preparation complete
...
#### Next planned work ‚Äî Week 2 safe computer control
...
- August 7, 2026 diagnosis from user screenshots: requests to list `C:\Games` were sent to the chat model, which has no filesystem-search tool. Its ‚Äúlist‚Äù and bracketed placeholders were fabricated text; Zen did not read or enumerate that folder. Folder-granted filename search remains unimplemented.
...
- Day 8 remaining work is now scope confirmation for the next capability: selected-folder filename search. The intended flow is native folder picker ‚Üí one-time folder grant ‚Üí preview of folder and query ‚Üí confirmation ‚Üí capped local filename/path results ‚Üí local activity record. It must not read file contents, leave the selected folder, or change files.
...
- Exact next recommended step: restart Zen, use **Activity ‚Üí Add an approved app** to approve one non-critical app, test cancellation then launch, restart Zen to confirm persistence, then remove its approval to verify revocation.
...
- Chat no longer forwards a File Explorer open request to the model. It displays local guidance to use Activity ‚Üí Open File Explorer, preventing the earlier false claim. The system prompt also explicitly forbids claims that chat executed a computer action.
...
- Read-aloud speed: added slower (0.8√ó), normal (1√ó), faster (1.2√ó), and fastest (1.4√ó) choices. The selected speed crosses the secure Electron bridge as a validated value and Piper receives the equivalent local `--length-scale`. A direct Ryan 1.2√ó Piper verification WAV was generated successfully (89,644 bytes) and deleted.
...
#### Day 6 ‚Äî reliability validation started
...
- Known issue: no active Day 5 blockers. The next recommended work is Day 6‚Äì7 Week 1 finishing: regression-test restart, model failure, saving, new chat, and deletion; fix any findings; prepare the release documentation and tag.
...
### Day 1 ‚Äî Complete and committed
...
### Day 2 ‚Äî Complete and committed
...
```text
I couldn‚Äôt reach the local model. Cannot read properties of undefined (reading 'chat')
```
...
- `README.md` ‚Äî quick start and completed status.
- `docs/PRD.md` ‚Äî product requirements.
- `docs/Architecture.md` ‚Äî technical architecture.
- `docs/Roadmap.md` ‚Äî four-week project roadmap.
- `apps/desktop/package.json` ‚Äî available application commands.
- `apps/desktop/src/main/main.js` ‚Äî Electron main process and Ollama request.
- `apps/desktop/src/main/preload.js` ‚Äî secure renderer-to-main bridge.
- `apps/desktop/src/renderer/renderer.js` ‚Äî chat state and UI behaviour.

## Recommended Immediate Steps

### Week 2 plan reconfirmed ‚Äî August 7, 2026
...
### Week 3 ‚Äî Memory and documents
...
### Week 4 ‚Äî Workflows and release
...
## Master 28-day checklist report ‚Äî August 7, 2026

- A complete Day 1‚Äì28 checklist was provided to the user from the recorded roadmap and completed-day commits. Days 1‚Äì10 are complete, with Day 10 committed as `b362047 feat: complete Day 10 local memory`; Days 11‚Äì28 are planned only.
...
## Day 11 ‚Äî document-import safety design complete (August 7, 2026)
...
- It specifies local-only extracted-text storage, no source-file copying or alteration, visible removal that deletes Zen's copy only, redacted activity logging, and a strict rule that document text and paths do not reach Ollama in Days 12‚Äì13.
...
## Day 13 ‚Äî local document search complete (August 7, 2026)
...
## Day 14 ‚Äî local document previews complete (August 7, 2026)
...
## Day 15 ‚Äî document Q&A design complete (August 8, 2026)
...
- Activity logging for `document-qa` is scoped to document display names, character count, and status only ‚Äî never question text, excerpt text, or the model's answer, consistent with the existing no-text-in-logs rule from Day 11.
...
- Exact next recommended step: implement Day 16 exactly to this design ‚Äî native search-result question entry point, confirmation preview, capped context assembly, and the fixed system instruction ‚Äî then run the Day 16 validation checklist in `docs/DocumentQA.md` before enabling it for regular use.
...
## Day 16 ‚Äî document Q&A implementation complete (August 8, 2026)
...
- Activity logging for `document-qa` now reflects the real chat outcome: entries move `requested` ‚Üí `completed`/`failed` based on `finishGeneration`, not on dispatch, and invalid/failed preparation also produces a logged entry (`rejected`/`failed`) where it previously did not. Logged fields remain limited to document name(s) and character count ‚Äî never question, excerpt, or answer text.
...
## Day 17 ‚Äî PDF text extraction design complete (August 9, 2026)
...
- Selected `pdfjs-dist` (Mozilla PDF.js, legacy Node build, Apache-2.0) for text-layer-only extraction ‚Äî no rendering, no embedded-script execution, no embedded-URI/network resolution. `pdf-parse` was considered and rejected as the primary choice due to inconsistent maintenance of its pinned dependency.
...
- PDF text will reach Ollama only through the existing Day 15/16 confirmation-gated Q&A flow, under the same 3-document/4,000-character cap ‚Äî no new model-access path.
...
## Day 18 follow-up ó closing the two open gaps (started August 9, 2026)

- Starting this pass: independently re-verified the prior session's claims against the real repo rather than trusting the write-up. `npm run check` passes as reported. However, decoding `apps/desktop/scanned.b64` directly and inspecting it byte-for-byte shows it does **not** end in `%%EOF` ó it ends in raw binary garbage, meaning the file itself is corrupted (most likely a leftover base64 transfer-truncation artifact, the same class of bug the prior session found and thought it had fully redone). It is not "a real scanned PDF whose structure happens to trigger PDF_MALFORMED" as the previous entry characterized it; it is a broken file. The `PDF_NO_TEXT_LAYER` code path is therefore still only covered by the synthetic fixture, not a real-world one.
- Plan for this pass: (1) rebuild `scanned.b64` as a genuine, valid, hand-verified image-only PDF (a real XObject-embedded raster with no font/text operator at all ó confirmed with pdfjs directly before it goes anywhere near the test suite) so `PDF_NO_TEXT_LAYER` gets real coverage; (2) tighten the corresponding `check-documents.js` assertion from "any `PDF_*` code" to the specific expected code, now that it can be trusted; (3) check current desktop state safely before attempting the still-open live Electron click-through, and only proceed if it's actually safe to do so (the prior session correctly declined once due to an active call/overlapping windows ó that judgment call stands and will be re-checked fresh, not overridden).
- Will update this section again once that work is done, with the actual outcome ó including if the live click-through remains undone for the same reason as before.
...
- **`scanned.b64` rebuilt correctly.** The old fixture is gone. The new one is a genuine,
  ...
  resource and no text-showing operator at all ó a faithful stand-in for what a scanner
...
- **Live GUI click-through: still genuinely open, and staying open for now.** Checked the
  ...
  open across several apps, and ó notably ó it is being actively watched live by the user
  ...
  Day 18 validation checklist that needs a human at the keyboard for a few minutes ó open the
...
## Day 18 ó fully closed: live click-through complete (August 9, 2026)

- The user ran the last open item themselves, in the real desktop app: imported
  `zen-day18-livetest.pdf` through the native picker (confirmed via screenshot ó the
...
- Day 18 (design in `docs/PDFExtraction.md`, implementation, the `scanned.b64` follow-up fix,
  and now this live pass) is complete end-to-end. No code changed in this update ó documentation
  only.
- Git state: this update only; no source changes.
- Exact next recommended step: begin Day 19 ó safe custom commands design (scope, confirmation
...
- Exact next recommended step: implement Day 21 exactly to `docs/Workflows.md` ó new `apps/desktop/src/main/workflows.js` module (storage, step/routing validation, the forward-only-index loop check, live re-resolution reusing `computer-control.js`/`custom-commands.js`), IPC wiring, a Workflows card on the Activity page showing branch structure, activity logging, and an automated test suite covering the Day 20 "before enabling" checklist ó then a live manual pass before it's considered complete, matching the Day 19 precedent of verifying in the running app before committing.
...
- Exact next recommended step: none for the original MVP. Continue only with user-directed
  code-signing setup, voice-license review, or a separately scoped v2.0 feature.

## v2.0 planning -- personal desktop agent (August 12, 2026)

- Added `docs/PersonalAgentPlan.md`: a 30-day v2.0 plan for Zen as a local-first personal
...
- No product code changed. Exact next recommended step: confirm the activation shortcut and the
  proposed personal-agent permission boundary, then begin v2.0 Day 1 design/threat-model work.
...
- **Correction to this same entry:** the initial pass of this audit checked `git log`/`git tag`
  ...
  (`docs: define Zen v2 personal agent plan`, `docs: add v2.0 personal desktop agent plan`, etc.)
...
## v2.0 -> v2.0 rename, and three build-plan answers folded in (August 13, 2026)
...
- The owner directed two changes this session: (1) answer three previously-open questions from
  ...
  to "Zen v2.0" everywhere it's used as the plan's name, since the owner will now update Zen
...
- **Renamed `docs/ZenV2Plan.md` -> `docs/ZenV2Plan.md`.** Every internal "v2.0" reference
  updated to "v2.0" (title, product brief, acceptance-standard heading, Day 1/29/30 headings).
...
- **Not folded in silently -- flagged as a new, real, unresolved conflict instead:** the owner's
  ...
  `docs/ZenV2Plan.md` laying out both real options (keep the no-shell boundary and cover
...
- **`docs/AgentContract.md`** updated: title and internal `v2.0` references changed to `v2.0`;
  now links to `docs/ZenV2Plan.md` by its new filename; Section 2 (risk tiers) carries the same
...
- **Naming collision resolved, not ignored:** `docs/PersonalAgentPlan.md` (the older, already-
  superseded v2.0-era exploration, flagged as an open cleanup item in an earlier entry below) had
  independently titled itself "Zen v2.0 personal desktop agent plan." Reassigning "v2.0" to the
...
- **`README.md` updated:** "Next planned version" section and the documentation index link both
  now say v2.0 and point at `docs/ZenV2Plan.md`, with a short note explaining the rename and
...
- **Historical entries in this file, above, were deliberately NOT rewritten.** Every prior
  ...
  file exists to preserve. Going forward, new entries use "v2.0."
- No product code changed. This is a documentation/naming/design-question update only.
- Git state: `docs/ZenV2Plan.md` (new, replacing deleted `docs/ZenV2Plan.md`),
...
- Exact next recommended step: owner decides the PowerShell/shell-scope question (two options
  laid out in `docs/ZenV2Plan.md`); once resolved, Day 2 (startup and tray lifecycle) can begin
...
- **PowerShell resolution:** the owner wants full local PowerShell control for themselves, but
  ...
  **"Full System Control (PowerShell)"** Settings toggle. Six specific design points were agreed
  and written into `docs/ZenV2Plan.md`'s "Resolved -- PowerShell / shell scope" section (which
...
- Git state: this session's PowerShell-resolution edits to `docs/ZenV2Plan.md` and
...
## Memory recall + auto-save ó external work, committed uncommitted-on-disk (August 14, 2026)
...
files. Not part of the v2.0 Day-by-day sequence ó v2.0 Day 2 (tray/startup lifecycle) has **not**
...
- `apps/desktop/src/renderer/renderer.js` (+392/-9) and `docs/Memory.md` (+9): Memory entries the
  user saved are now sent to Ollama as chat context automatically (capped ~4,000 chars) ó this
...
- `apps/desktop/src/renderer/index.html` (+6/-2): Memory page copy rewritten to disclose the new
  behavior plainly ó tells the user memories are now shared with the local model automatically
  and picked up from chat.
- `docs/Voice.md` (+3/-2): unrelated doc fix ó documents the existing per-voice read-aloud speed
...
convention ó flagged to Sufi, not auto-corrected.
...
Exact next recommended step: still Day 2 -- startup and tray lifecycle, per `docs/ZenV2Plan.md`
