...
files. Not part of the v2.0 Day-by-day sequence — v2.0 Day 2 (tray/startup lifecycle) has **not**
...
**Last Verified Commit:** `9a4c6de` (superseded -- see entry below).

Exact next recommended step (superseded): still Day 2 -- startup and tray lifecycle, per
`docs/ZenV2Plan.md` -- once Sufi decides whether to push this memory change as-is or add a
confirmation gate first.

## v2.0 sprint kickoff -- Block A: repo sync & rename (August 15, 2026)

Executing `docs/ZenV2-2Day-Sprint-Plan.md` per `docs/AgentKickoffBrief.md`, starting Block A
Step 1.

**What changed:**
- Step 1: Tier 1 verification (`git rev-parse HEAD`, `git status --porcelain`) confirmed clean
  against the kickoff brief's stated state -- HEAD `375bd03`, 0 ahead/0 behind `origin/zen-1.1`,
  no unexpected drift. (The prior entry's "not pushed" note for `9a4c6de` was stale prose --
  that commit and its follow-up were both already on `origin` by session start.)
- Step 2: renamed branch `zen-1.1` -> `zen-2.0` (local + remote, upstream tracking set), deleted
  old `origin/zen-1.1`.
- Step 3: renamed `docs/ZenV1_1Plan.md` -> `docs/ZenV2Plan.md`; literal repo-wide find/replace of
  "v1.1" -> "v2.0" across every tracked file containing it (`HANDOFF.md` incl. historical log
  entries, `README.md`, `docs/AgentContract.md`, `docs/PersonalAgentPlan.md`, `docs/ZenV2Plan.md`),
  per the owner's explicit historical-rewrite decision. Excluded this sprint's own kickoff/plan
  docs (`docs/AgentKickoffBrief.md`, `docs/ZenV2-2Day-Sprint-Plan.md`) from the blind replace
  since their "v1.1" mentions narrate the rename itself -- a literal swap there would have made
  those sentences self-contradictory. Flagged as a judgment call.
- Step 4: confirmed `docs/AgentModePlan.md` / `docs/AgentModeChecklist.md` still absent
  (`Test-Path`, both `False`).
- Step 5: finalized `docs/AgentContract.md` -- Section 7 now holds the complete v2.0 action
  registry (all 16 actions + `noop.wait`) with risk tier and confirmation type per action;
  `docs/ActionRegistrySkeleton.md` updated to match (Status: Schema drafted). Cleaned up a
  self-contradictory "renamed from v2.0 to v2.0" sentence left over from the literal replace.
  Flagged one open gap: `click-control` / `type-into-field` can't have their sensitive sub-cases
  (clicking Delete/Send, typing into a password field) fixed at registration time the way
  `run-powershell`'s trigger-pattern classifier does -- registered `routine` as an interim
  default, with the gap called out for Block F Step 25 to close.
- Step 6: committed as `13f4371` (`docs: rename v1.1 to v2.0, finalize agent contract`), pushed
  to `origin/zen-2.0`.

**Corruption caught and fixed post-push:** the Step 3 find/replace was first run via PowerShell
`Get-Content -Raw` / `Set-Content`, which silently mis-decoded non-ASCII characters (em dashes,
smart quotes) in the affected files under Windows PowerShell 5.1's default encoding, and that
corrupted version was committed and pushed as `13f4371` before the damage was caught. Fixed via a
follow-up commit: restored the 5 affected files from the pre-corruption commit (`375bd03`) using
`git checkout`, redid the find/replace with explicit UTF-8 (no-BOM, except `HANDOFF.md` which
kept its original BOM) via .NET `File.ReadAllText`/`WriteAllText`, verified zero corrupted bytes
and clean diffs against `375bd03`, then re-applied the Step 5 `AgentContract.md` edits (which the
restore had also reverted). Chose a fix-forward commit over rewriting the already-pushed history
-- flagged per `INSTRUCTIONS.md` Section 5 in case a force-push correction was preferred instead.

**Git state:** `zen-2.0`, working tree otherwise clean except pre-existing unrelated untracked
items (`.backups/`, `.cursorrules.txt`, `deliverables/`) -- not touched, out of scope.

**Known issues / open items:** the `click-control` sensitive-detection gap above; owner
confirmation still pending on that interim default and on the `blocked` task-state addition
(`docs/AgentContract.md` Section 1, flagged in an earlier pass, not yet resolved).

**Exact next recommended step:** Block B (instant invocation) -- Steps 7-11: system tray
